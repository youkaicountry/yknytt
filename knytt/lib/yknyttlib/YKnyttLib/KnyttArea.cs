...
namespace YKnyttLib
{
    public class KnyttArea
    {
        ...
        private void loadFromStream(Stream map)
        {
            ...
            //GD.Print(string.Format("Pos: ({0}, {1}), T: ({2}, {3})", this.Position.x, this.Position.y, this.TilesetB, this.TilesetA));
        }
        ...
        private static byte[] parseByteArray(Stream map, int size)
        {
            ...
            {
                int bytesRead = map.Read(data, totalBytesRead, size - totalBytesRead);
